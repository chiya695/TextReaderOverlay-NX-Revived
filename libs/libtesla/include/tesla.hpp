...
namespace tsl {
    ...
    namespace gfx {
        ...
        public:
            ...
            std::string limitStringLength(std::string string, bool monospace, float fontSize, s32 maxLength) {
                ...
                do {
                    ...
                    currFont = this->fontForCodepoint(currCharacter);
        ...
        private:
            stbtt_fontinfo* fontForCodepoint(u32 codepoint) {
                if (stbtt_FindGlyphIndex(&this->m_extFont, static_cast<int>(codepoint)) != 0)
                    return &this->m_extFont;
                if (stbtt_FindGlyphIndex(&this->m_stdFont, static_cast<int>(codepoint)) != 0)
                    return &this->m_stdFont;
                if (this->m_hasLocalFont && stbtt_FindGlyphIndex(&this->m_localFont, static_cast<int>(codepoint)) != 0)
                    return &this->m_localFont;
                for (size_t i = 0; i < this->m_fallbackFonts.size(); ++i) {
                    if (this->m_hasFallbackFont[i] &&
                        stbtt_FindGlyphIndex(&this->m_fallbackFonts[i], static_cast<int>(codepoint)) != 0)
                        return &this->m_fallbackFonts[i];
                }
                return &this->m_stdFont;
            }
            ...
            std::array<stbtt_fontinfo, 4> m_fallbackFonts{};
            std::array<bool, 4> m_hasFallbackFont{};
            ...
            Result initFonts() {
                ...
                constexpr std::array<PlSharedFontType, 4> fallbackTypes = {
                    PlSharedFontType_ChineseSimplified,
                    PlSharedFontType_ExtChineseSimplified,
                    PlSharedFontType_ChineseTraditional,
                    PlSharedFontType_KO,
                };
                static std::array<PlFontData, 4> fallbackFontData{};
                for (size_t i = 0; i < fallbackTypes.size(); ++i) {
                    if (R_SUCCEEDED(plGetSharedFontByType(&fallbackFontData[i], fallbackTypes[i]))) {
                        fontBuffer = reinterpret_cast<u8*>(fallbackFontData[i].address);
                        const int fontOffset = stbtt_GetFontOffsetForIndex(fontBuffer, 0);
                        if (fontOffset >= 0 && stbtt_InitFont(&this->m_fallbackFonts[i], fontBuffer, fontOffset))
                            this->m_hasFallbackFont[i] = true;
                    }
                }
