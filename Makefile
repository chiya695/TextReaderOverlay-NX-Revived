...
release: all
	...
	if [ -d package ]; then cp -r package/. sdfiles/switch/.overlays/TextReaderOverlay/; fi
	if command -v zip >/dev/null 2>&1; then cd sdfiles/ && zip -r ../$(RELEASE_ZIP) * && cd ..; else cd sdfiles/ && bsdtar -a -cf ../$(RELEASE_ZIP) . && cd ..; fi
	rm -rf sdfiles/
